...
def mos_disp(v):
    ...
    (적정가-현재가)/적정가는 주가가 적정가치를 크게 웃돌면 -1000%처럼
    읽기 어려운 값으로 발산한다. 다만 '발산 = 산출 실패'는 아니다 —
    적정가치가 정상 산출됐는데 주가가 그보다 훨씬 높을 뿐일 수도 있다.
    그래서 실패로 단정하지 않고 배수로 환산해 보여주고, 배수가 비상식적일
    때만 오류를 의심하도록 한다. (주가/적정가 = 1 - mos)
    """
    if v is None:
        return "N/A"
    if v >= -1.0:
        return pct(v)
    ratio = 1 - v
    tail = " ⚠ 산출 오류 의심" if ratio > 20 else ""
    return f"{v*100:,.0f}% · 주가 = 적정가치의 {ratio:.1f}배{tail}"
